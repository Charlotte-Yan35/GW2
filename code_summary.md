...
## 2. experiment_ratio_simplex

**研究问题**：在 50 节点微电网中，发电/消费/被动节点的配比 (r_g, r_c, r_p) 如何影响不同拓扑结构（WS、RGG、SBM、CP）下的同步临界耦合 κ_c？

### 2.1 目录结构

```
experiment_ratio_simplex/
├── compute_ratio_simplex_kappa.py  # 四族拓扑 × 单纯形配比扫描
├── plot_ratio_simplex_kappa.py     # 2×2 三元热力图
├── plot_topology_structures.py     # 2×2 拓扑结构可视化
├── cache/                          # 计算缓存
└── figures/                        # 输出图片
```
...
| 拓扑 | 生成方式 | κ_c 起始值 |
|------|---------|-----------|
| **WS** (Watts-Strogatz) | 49 节点小世界图，q=0.1，PCC 随机连 4 个节点 | 10 |
| **RGG** (Random Geometric Graph) | [0,1]² 平面，二分搜索半径使平均度 ≈ K_TARGET | 30 |
| **SBM** (Stochastic Block Model) | 4 社区，p_in : p_out = 8:1，PCC 连每个块 1 节点 | 30 |
| **CP** (Core-Periphery) | 20% 核心，密度分层，PCC 只连核心 | 50 |
...
| `PMAX` | 1.0 | 最大功率幅值 |
| `R` | 30 | 每组合实现次数 |
| `step` | 0.1 | 单纯形网格步长（共 45 个格点） |

**单纯形约束**：$r_g + r_c + r_p = 1$，$r_g \geq 0.1$，$r_c \geq 0.1$

### 2.4 角色分配与功率注入

```
n_g 个发电节点：P = +Pmax / n_g
n_c 个消费节点：P = -Pmax / n_c
n_p 个被动节点：P = 0
PCC 节点：P = 0
总功率平衡：sum(P) = 0
```
...
二分搜索 + 多初始条件重试：
1. 从 `kappa_start` 开始递降步长
2. 收敛 → warm-start 继续降低 κ
3. 不收敛 → 尝试零初始/小扰动初始条件
4. 步长 < 容差时终止

**收敛判定**：稳态残差 $\|\text{residual}\|_2 < 10^{-4}$

### 2.6 关键函数

| 函数 | 作用 |
|------|------|
| `build_ratio_grid(step)` | 生成三元单纯形格点 |
| `ratios_to_counts(rg, rc, rp, n)` | 比例 → 整数节点数 |
| `generate_ws/rgg/sbm/cp(seed)` | 各族网络生成器 |
| `assign_roles(ng, nc, n_households, seed)` | 功率注入向量 |
| `_find_kappa_c(A, P, n, kappa_start)` | 二分搜索 κ_c |
| `run_experiment(R, step)` | 主实验，多进程并行，断点续算 |

### 2.7 绘图

| 图表 | 输出文件 | 内容 |
|------|---------|------|
| 配比热力图 | `figures/ratio_simplex_kappa_2x2.png/pdf` | 2×2 面板，4 种拓扑的 log₁₀(κ_c) 三角插值热力图 |
| 拓扑结构 | `figures/topology_structures_2x2.png` | 2×2 面板，各族代表性网络可视化 |

**热力图坐标转换**（重心 → 直角）：$x = 0.5 r_g + r_p, \quad y = \frac{\sqrt{3}}{2} r_g$

### 2.8 缓存文件

| 文件 | 内容 |
|------|------|
| `cache/raw_results.csv` | 原始记录，支持断点续算 |
| `cache/agg_results.csv` | 按 (r_g, r_c, r_p, family) 聚合的 κ_c 均值/标准差 |
| `cache/metadata.json` | 实验配置元数据 |
