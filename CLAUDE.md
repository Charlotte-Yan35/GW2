...
reproduction/          # 论文图表复现 (Figure 1–6, S2, WS topology)
  ...
  output/                # 输出图片
2nodes/                # 两节点稳定性分析（分岔图、相图、时间序列）
household/             # 家庭用电 / 光伏 / 微电网分析
  consumer/              # LCL 用电统计分析（含 Parquet 转换工具）
  generation/            # 光伏发电分析
  node/                  # 家庭微电网模型（含储能）
Topology2.0/           # 拓扑与韧性扩展实验
  ws_topology_effects/   # WS 拓扑效应（已模块化重构）
    ws_stability/          # 稳定性计算：κ_c 映射、Lorenz/Gini 分析、多配比对比
...
- **NEVER delete `Q1Topology/` 中的缓存目录**（`ws_kappa_c/cache/`、`ws_percolation/cache/`），同理。
...
MPLBACKEND=Agg .venv/bin/python ratio_scan/plot_stability_extra.py
MPLBACKEND=Agg .venv/bin/python ratio_scan/plot_passive_impact.py

# 拓扑扩展实验
MPLBACKEND=Agg .venv/bin/python Topology2.0/ws_topology_effects/run_all.py
MPLBACKEND=Agg .venv/bin/python Topology2.0/experiment_ratio_simplex/compute_ratio_simplex_kappa.py
MPLBACKEND=Agg .venv/bin/python Topology2.0/experiment_ratio_simplex/plot_ratio_simplex_kappa.py
MPLBACKEND=Agg .venv/bin/python Topology2.0/cascade_resilience/experiment_recovery_time_ws.py
MPLBACKEND=Agg .venv/bin/python Topology2.0/cascade_resilience/plots_recovery_panels.py
```

## Code Conventions

- 脚本使用中文注释
- 缓存文件格式：`.npz`（numpy compressed）、`.pkl`（pickle）、`.csv`（表格数据）、`.parquet`（大规模表格）
